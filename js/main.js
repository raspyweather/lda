...
function canvasClick(event) {
    ...
    const x = (event.pageX - canvas.offsetLeft) / canvas.width;
    const y = (event.pageY - canvas.offsetTop) / canvas.height;
...
function undo() {
    ...
    const lastColor = pointHistory[pointHistory.length - 1];
...
function drawCanvas() {
    ...
    plotPoints(bluePoints, blueColor);
    plotPoints(redPoints, redColor);
...
function cov(a) {
    ...
    const n = a.size()[0];
    const x = math.subtract(a, math.multiply((1 / n), math.ones(n, 1), math.ones(1, n), a));
    const y = math.multiply(math.transpose(x), x);
...
function mean(a) {
    ...
    let result = math.zeros(1, a.size()[1]);
    const n = a.size()[0];
    for (let i = 0; i < n; i++) {
...
function computeLDA() {
    ...
    const scaledBlue = scalePointsToCanvas(bluePoints);
    const scaledRed = scalePointsToCanvas(redPoints);

    // Compute LDA
    const c1 = math.matrix(scaledBlue);
    const c2 = math.matrix(scaledRed);
...
function drawSeparationLine(theta, b) {
    ...
    const y1 = (theta.get([0, 0]) * x1 + b) / (-1 * theta.get([1, 0]));
    const y2 = (theta.get([0, 0]) * x2 + b) / (-1 * theta.get([1, 0]));
    line = {x1, x2, y1, y2};
